...
经过多次调整：
1. 第一次：旋转逻辑反了（红方旋转 180°，黑方不旋转）
2. 第二次：修正为黑方旋转 180°，红方不旋转，但仍然不对
3. 第三次：尝试黑方 90°，红方 -90°，还是不对
4. 第四次（最终）：用户明确指出"黑色棋子在上方字体旋转 -90度，红色棋子在下方字体旋转 90度"

**根本原因：**
Three.js 中圆柱体顶部纹理的默认朝向需要特定的旋转角度才能使文字正确朝向楚河汉界。经过多次尝试，最终确定正确的旋转角度是：
- 黑方：-90°（-π/2）
- 红方：90°（π/2）
...
if (!isRed && row < 5) {
  // 黑方棋子在上半部分，旋转 -90 度
  pieceMesh.rotation.y = -Math.PI / 2;  // ✅ 最终正确值
} else if (isRed && row >= 5) {
  // 红方棋子在下半部分，旋转 90 度
  pieceMesh.rotation.y = Math.PI / 2;   // ✅ 最终正确值
} else if (!isRed && row >= 5) {
...
**关键修正历程：**
- ❌ 第一次尝试：红方 180°，黑方 0°（完全错误）
- ❌ 第二次尝试：黑方 180°，红方 0°（仍然错误）
- ❌ 第三次尝试：黑方 90°，红方 -90°（方向反了）
- ✅ 第四次修正：黑方 -90°，红方 90°（正确）

**验证方法：**
1. 黑方棋子（上半部分）的文字应该正确朝向楚河汉界
2. 红方棋子（下半部分）的文字应该正确朝向楚河汉界
